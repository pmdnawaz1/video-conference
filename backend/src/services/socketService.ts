...
export function initializeSocketIO(io: SocketIOServer) {
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('create-room', (data: { roomName?: string; maxUsers?: number }) => {
    ...
    socket.on('join-room', (data: { roomId: string }) => {
      ...
      console.log(`🚪 User ${user.name} joined room ${roomId} (${room.users.size}/${room.maxUsers})`);
    });
    ...
    socket.on('offer', (data: { roomId: string; targetUserId: string; offer: any }) => {
      ...
      console.log(`📞 Offer sent from ${socket.id} to ${targetUserId} in room ${roomId}`);
    });
    ...
    socket.on('answer', (data: { roomId: string; targetUserId: string; answer: any }) => {
      ...
      console.log(`📞 Answer sent from ${socket.id} to ${targetUserId} in room ${roomId}`);
    });
  ...
  // Helper functions
  function updateMediaState(socket: Socket, roomId: string, mediaState: Partial<SocketUser>) {
    const user = users.get(socket.id);
    const room = rooms.get(roomId);
