 ...
 */
export function authorizeClient(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  if (!req.user) {
    return res.status(401).json({
      error: 'Authentication required',
      message: 'User not authenticated'
 ...
 */
export function handleCorsAuth(req: Request, res: Response, next: NextFunction) {
 ...
 */
export function logAuthenticatedRequests(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  const user = req.user;
  ...
  if (user) {
    console.log(`🔐 [${method}] ${url} - User: ${user.email} (${user.role}) - IP: ${ip}`);
  } else {
