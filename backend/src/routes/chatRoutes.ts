 ...
 * @access Private
 */
router.post('/messages', authenticate, async (req: AuthenticatedRequest, res) => {
  try {
    if (!req.user) {
    ...
    if (!content || !meetingId) {
      return res.status(400).json({ error: 'Content and meetingId are required' });
    }
 ...
 * @access Private
 */
router.post('/files', authenticate, upload.single('file'), async (req: AuthenticatedRequest, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'User not authenticated' });
    }
 ...
 * @access Private
 */
router.get('/files/:fileName', authenticate, async (req: AuthenticatedRequest, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'User not authenticated' });
    }
 ...
 * @access Private
 */
router.get('/messages/:meetingId', authenticate, async (req: AuthenticatedRequest, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'User not authenticated' });
    }
    ...
    const chatService = getChatService();
    const result = await chatService.getChatHistory(meetingId, req.user.id, options);
 ...
 * @access Private
 */
router.post('/messages/:messageId/reactions', authenticate, async (req: AuthenticatedRequest, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'User not authenticated' });
    }
 ...
 * @access Private
 */
router.put('/messages/:messageId', authenticate, async (req: AuthenticatedRequest, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'User not authenticated' });
    }
 ...
 * @access Private
 */
router.delete('/messages/:messageId', authenticate, async (req: AuthenticatedRequest, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'User not authenticated' });
    }
    ...
    const chatService = getChatService();
    await chatService.deleteMessage(messageId, req.user.id, moderation === 'true');
 ...
 */
router.post('/cleanup', authenticate, async (req: AuthenticatedRequest, res) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'User not authenticated' });
    }
