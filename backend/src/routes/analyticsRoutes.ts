...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      let roomStats = null;
      if (isAdmin) {
        roomStats = await roomManagementService.getRoomStats(
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const { period = "daily", days = "30", userId } = req.query;
      ...
      const analyticsArray = Object.values(analytics).sort(
        (a: any, b: any) =>
      ...
      const summary = {
        ...
        totalDuration: meetingAnalytics.reduce(
        ...
        averageDuration:
          meetingAnalytics.length > 0
            ? Math.round(
                meetingAnalytics.reduce(
                  (sum, ma) => sum + ma.actualDuration,
                  0,
                ) / meetingAnalytics.length,
              )
            : 0,
        totalParticipants: meetingAnalytics.reduce(
        ...
        averageParticipants:
          meetingAnalytics.length > 0
            ...
            : 0,
        totalMessages: meetingAnalytics.reduce(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const userAnalytics = await prisma.userAnalytics.findMany({
        ...
        orderBy: [{ date: "desc" }, { totalMeetingDuration: "desc" }],
      });
      ...
      const userEngagement = topUsers.map((ua) => ({
        ...
        averageDuration:
          ua.meetingsCreated + ua.meetingsJoined > 0
            ? Math.round(
                ua.totalMeetingDuration /
                  (ua.meetingsCreated + ua.meetingsJoined),
              )
            : 0,
      ...
      res.json({
        ...
        analytics: {
          period,
          months: monthCount,
          data: Object.values(analytics).sort(
            (a: any, b: any) =>
              new Date(a.date).getTime() - new Date(b.date).getTime(),
          ),
          ...
          summary: {
            ...
            totalMeetings: userAnalytics.reduce(
              (sum, ua) => sum + ua.meetingsCreated + ua.meetingsJoined,
              0,
            ),
            totalDuration: userAnalytics.reduce(
              (sum, ua) => sum + ua.totalMeetingDuration,
              0,
            ),
            totalMessages: userAnalytics.reduce(
              (sum, ua) => sum + ua.messagesSet,
              0,
            ),
          },
        },
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      // Calculate trends
      const totalMeetings = usageStats.reduce(
        (sum, day) => sum + day.meetingsCreated,
        0,
      );
      const totalMessages = usageStats.reduce(
        (sum, day) => sum + day.messagesSet,
        0,
      );
      ...
      res.json({
        ...
        analytics: {
          ...
          summary: {
            ...
            peakDay: usageStats.reduce(
              (max, day) =>
                day.meetingsCreated > max.meetingsCreated ? day : max,
              usageStats[0],
            ),
          },
        },
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const { type = "meetings", format = "json", days = "30" } = req.query;
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      res.json({
        ...
        realTimeMetrics: {
          ...realTimeMetrics,
          participants: realTimeMetrics?.participants
            ? Array.from(realTimeMetrics.participants)
            : [],
      ...
      const insights = {
        ...
        recommendations: [
          {
            ...
            description:
              "Consider implementing interactive features like polls or breakout rooms",
            priority: "medium",
            impact: "high",
          },
          {
            ...
            description:
              "Monitor connection quality trends and consider CDN implementation",
            priority: "low",
            impact: "medium",
          },
          {
            ...
            description:
              "Create guided tours for new users to improve adoption",
            priority: "high",
            impact: "high",
          },
        ],
        predictions: {
          ...
          capacityRecommendation:
            dashboard.overview.activeMeetings > 10 ? "scale-up" : "current",
        },
      };
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const qualityDistribution = connectionQualityStats.reduce(
        (acc, stat) => {
      ...
      for (let i = hourCount - 1; i >= 0; i--) {
        ...
        const hourDropouts = dropoutRates.filter(
          (dr) =>
            dr.meeting.createdAt >= hour && dr.meeting.createdAt < nextHour,
        );

        const avgDropoutRate =
          hourDropouts.length > 0
            ? hourDropouts.reduce((sum, dr) => sum + dr.dropoutRate, 0) /
              hourDropouts.length
            : 0;
          ...
          summary: {
            ...
            averageDropoutRate:
              dropoutRates.length > 0
                ? Math.round(
                    (dropoutRates.reduce((sum, dr) => sum + dr.dropoutRate, 0) /
                      dropoutRates.length) *
                      100,
                  ) / 100
                : 0,
