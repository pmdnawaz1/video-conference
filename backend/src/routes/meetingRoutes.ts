...
const createRateLimit = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes
  maxRequests: 10, // 10 meeting creations per window
 ...
 */
router.post('/', authenticate, createRateLimit, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user) {
    ...
    if (scheduledStartTime && scheduledEndTime && new Date(scheduledEndTime) <= new Date(scheduledStartTime)) {
      return res.status(400).json({
...
router.get('/:meetingId', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const meeting = await meetingManagementService.getMeetingDetails(meetingId, req.user.id);
...
router.put('/:meetingId', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const meeting = await meetingManagementService.updateMeeting(meetingId, updates, req.user.id);
 ...
 * Start a meeting
 */
router.post('/:meetingId/start', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Not authenticated' });
    }
    ...
    const result = await meetingManagementService.startMeeting(meetingId, req.user.id);
 ...
 * End a meeting
 */
router.post('/:meetingId/end', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Not authenticated' });
    }
    ...
    const meeting = await meetingManagementService.endMeeting(meetingId, req.user.id);
 ...
 */
router.post('/:meetingId/participants', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Not authenticated' });
    }
 ...
 */
router.delete('/:meetingId/participants/:userId', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Not authenticated' });
    }
    ...
    const removed = await meetingManagementService.removeParticipant(userId, meetingId, req.user.id);
 ...
 */
router.post('/:meetingId/participants/:userId/approve', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Not authenticated' });
    }
    ...
    const participant = await meetingManagementService.approveParticipant(userId, meetingId, req.user.id);
 ...
 */
router.get('/', authenticate, generalRateLimit, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Not authenticated' });
    }
    ...
    const result = await meetingManagementService.getUserMeetings(req.user.id, filters);
 ...
 */
router.get('/:meetingId/analytics', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Not authenticated' });
    }
    ...
    const hasAccess = await meetingManagementService.checkMeetingAccess(meetingId, req.user.id);
    if (!hasAccess) {
      return res.status(403).json({
        error: 'Access denied',
        message: 'You do not have permission to view analytics for this meeting'
      });
    }

    const analytics = await meetingManagementService.calculateMeetingAnalytics(meetingId);
 ...
 */
router.get('/:meetingId/access', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Not authenticated' });
    }
 ...
 */
router.post('/join/:meetingId', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Not authenticated' });
    }
 ...
 */
router.post('/leave/:meetingId', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Not authenticated' });
    }
    ...
    const removed = await meetingManagementService.removeParticipant(req.user.id, meetingId, req.user.id);

    res.json({
      success: true,
      message: removed ? 'Successfully left meeting' : 'You were not in this meeting'
    });
  } catch (error) {
