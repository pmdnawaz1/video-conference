  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      if (!reactionsService) {
        return res
          .status(503)
          .json({ error: "Reactions service not available" });
      }
...
router.get(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      // Process analytics
      const reactionsByTime = reactions.reduce(
        (acc, reaction) => {
      ...
      res.json({
        ...
        analytics: {
          ...
          reactionsByTime,
          reactionsByEmoji,
          reactionsByUser: Object.entries(reactionsByUser).map(
            ([userId, data]) => ({
              userId,
              ...(data as any),
            }),
...
router.get(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      res.status(500).json({
        error:
          error instanceof Error ? error.message : "Failed to get live state",
      });
    }
  },
);
...
router.post(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      res.status(500).json({
        error:
          error instanceof Error ? error.message : "Failed to clear reactions",
      });
    }
  },
);
...
router.get(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const emojiUsage = reactions.reduce(
        (acc, reaction) => {
          acc[reaction.emoji] = (acc[reaction.emoji] || 0) + 1;
          return acc;
        },
      ...
      const dailyActivity = reactions.reduce(
        (acc, reaction) => {
      ...
      res.json({
        ...
        stats: {
          summary: {
            ...
            favoriteEmoji:
              Object.entries(emojiUsage).sort(
                ([, a], [, b]) => b - a,
              )[0]?.[0] || null,
            timeframe,
          },
          emojiUsage,
          dailyActivity,
          meetingActivity: Object.entries(meetingActivity).map(
            ([meetingId, data]) => ({
              meetingId,
              ...(data as any),
            }),
...
router.get(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const startDate =
        timeframes[timeframe as keyof typeof timeframes] || timeframes["30d"];
