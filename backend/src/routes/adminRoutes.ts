...
router.get("/dashboard", async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const dashboardStats =
      await adminDashboardService.getDashboardStats(clientId);
...
router.get("/users", async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const userManagement = await adminDashboardService.getUsersManagement(
      clientId,
      {
        page,
        limit,
...
router.post("/users", async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const { email, firstName, lastName, displayName, role, clientId } =
      req.body;
    ...
    const result = await adminDashboardService.createUser(
...
router.put(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const updatedUser = await adminDashboardService.updateUser(
        userId,
        currentUser.clientId,
        updates,
      );
...
router.get("/meetings", async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const analytics = await adminDashboardService.getMeetingAnalytics(
      clientId,
      dateRange,
    );
...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const tokens = await adminDashboardService.impersonateUser(
        userId,
        req.user.id,
      );
      ...
      switch (operation) {
        ...
        default:
          return res.status(400).json({
            ...
            message:
              "Supported operations: cleanup_old_rooms, cleanup_old_meetings, reset_user_sessions",
          });
      }

      console.log(
        `🔧 System maintenance performed: ${operation} by admin ${req.user.id}`,
        result,
      );
