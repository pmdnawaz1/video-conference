...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const isAdmin =
        req.user.role && ["ADMIN", "SUPER_ADMIN"].includes(req.user.role);
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const totalMeetings = analytics.reduce(
        (sum, a) => sum + a.meetingsCreated + a.meetingsJoined,
        0,
      );
      const totalDuration = analytics.reduce(
        (sum, a) => sum + a.totalMeetingDuration,
        0,
      );
      const totalMessages = analytics.reduce(
        (sum, a) => sum + a.messagesSet,
        0,
      );
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const { role, isActive = "true", limit = "20", offset = "0" } = req.query;
