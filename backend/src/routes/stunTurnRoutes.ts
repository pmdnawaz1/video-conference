    ...
    try {
      ...
      const options = {
        ...
        bandwidthRequirement: bandwidthRequirement
          ? parseInt(bandwidthRequirement as string)
          : undefined,
    ...
    try {
      ...
      const options = {
        ...
        latencyRequirement: latencyRequirement
          ? parseInt(latencyRequirement as string)
          : undefined,
        costOptimized: costOptimized === "true",
      };

      const optimalProviders =
        await stunTurnService.getOptimalConfiguration(options);
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const summary = {
        ...
        healthyProviders: providerStats.filter(
          (p) => !p.healthCheck || p.healthCheck.isHealthy !== false,
        ).length,
        totalConnections: connectionStats.reduce(
          (sum, stat) => sum + stat.connectionsCount,
          0,
        ),
        totalBandwidth: connectionStats.reduce(
          (sum, stat) => sum + stat.bytesTransferred,
          0,
        ),
        averageSuccessRate:
          connectionStats.length > 0
            ? connectionStats.reduce((sum, stat) => sum + stat.successRate, 0) /
              connectionStats.length
            : 100,
        averageLatency:
          connectionStats.length > 0
            ? connectionStats.reduce(
                (sum, stat) => sum + stat.averageLatency,
                0,
              ) / connectionStats.length
            : 0,
      };
...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      console.log(
        `🧪 Testing TURN connectivity requested by admin ${req.user.email}`,
      );
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (healthStatus.providers.active === 0) {
        ...
        healthStatus.providers.healthy / healthStatus.providers.active <
        0.5
      ) {
