    ...
    (set, get) => ({
      ...
      fetchDashboardData: async () => {
        ...
        try {
          ...
          // For regular users, get their personal dashboard data
          // For admins, get the admin dashboard
          const isAdmin =
          ...
          if (isAdmin) {
            // Admin dashboard - get from admin endpoint
            const response = await fetch(
            ...
            if (response.ok && result.success) {
              const adminData = result.dashboard || result.data || {};

              // Transform admin dashboard data
              let recentActivityArray = [];
              if (adminData.recentActivity) {
                const activity = adminData.recentActivity;
                const recentMeetings = (activity.recentMeetings || []).map(
                  (meeting) => ({
                    type: "meeting",
                    description: `Meeting "${meeting.title || "Untitled"}" ${meeting.status === "ACTIVE" ? "started" : "created"}`,
                    timestamp:
                      meeting.createdAt ||
                      meeting.updatedAt ||
                ...
                const recentUsers = (activity.recentUsers || []).map(
                  (user) => ({
                    type: "user",
                    description: `${user.displayName || user.firstName + " " + user.lastName || user.email} joined`,
                    timestamp: user.createdAt || new Date().toISOString(),
                    ...user,
                  }),
                );

                recentActivityArray = [...recentMeetings, ...recentUsers]
            ...
            // Regular user dashboard - get from analytics and meetings
            const [analyticsResponse, meetingsResponse] = await Promise.all([
              fetch(
                `${import.meta.env.VITE_API_BASE_URL}/api/analytics/usage?days=7`,
                {
                  headers: get().getAuthHeaders(),
                },
              ),
              fetch(
                `${import.meta.env.VITE_API_BASE_URL}/api/users/${user.id}/meetings?limit=50&offset=0`,
                {
                  headers: get().getAuthHeaders(),
                },
              ),
            ...
            const [analyticsResult, meetingsResult] = await Promise.all([
            ...
            // Transform user data
            const meetings =
              meetingsResult.success && meetingsResult.meetings
            ...
            const todaysMeetings = meetings.filter((m) => {
              ...
              return new Date(m.scheduledStartTime).toDateString() === today;
            });
            ...
            const activeMeetings = meetings.filter((m) => {
              return m.status === "ACTIVE" || m.status === "IN_PROGRESS";
            });

            const completedMeetings = meetings.filter((m) => {
              return m.status === "COMPLETED" || m.status === "ENDED";
            });

            const recentActivity = meetings.slice(0, 5).map((meeting) => ({
              type: "meeting",
              description: `Meeting "${meeting.title}" ${meeting.status.toLowerCase()}`,
            ...
            dashboardData = {
              ...
              quickStats: {
                ...
                todaysCount: todaysMeetings.length,
      ...
      fetchAnalytics: async (timeframe = "month") => {
        ...
        try {
          ...
          const response = await fetch(
            `${import.meta.env.VITE_API_BASE_URL}/api/users/${user.id}/analytics?period=daily&limit=30`,
            {
              headers: get().getAuthHeaders(),
            },
          );
          ...
          if (response.ok && result.success) {
            const rawData = result.analytics?.data || [];

            // Perform comprehensive data transformation
            const totalMeetings = rawData.reduce(
              (sum, item) => sum + (item.meetingsJoined || 0),
              0,
            );
            const totalMinutes = rawData.reduce(
              (sum, item) => sum + (item.totalMeetingDuration || 0),
              0,
            );
            ...
            const transformedAnalytics = {
              ...
              averageParticipation:
                totalMeetings > 0
                  ? rawData.reduce(
                      (sum, item) => sum + (item.averageParticipation || 0),
                      0,
                    ) / totalMeetings
                  : 0,
              engagementScore: rawData.length > 0 ? 75 : 0, // Placeholder
              lastActivity: rawData.length > 0 ? rawData[0].date : null,
              participationTrends: rawData.map((item) => ({
                date: item.date,
                meetingsAttended: item.meetingsJoined,
                participationRate: item.averageParticipation,
                engagementScore: 75, // Placeholder
                value: item.meetingsJoined || 0,
              })),
              // Add mock data for missing fields until backend provides them
              engagementBreakdown: {
                speaking: 40,
                messages: 30,
                reactions: 20,
              ...
              meetingTypes: { SCHEDULED: 10, INSTANT: 5, RECURRING: 2 },
              weeklyPattern: {
                monday: 5,
                tuesday: 8,
                wednesday: 6,
              ...
              hourlyPattern: { 10: 5, 11: 8, 14: 6, 15: 7 },
              goalProgress: 60, // Mock data
              peakHour: "11:00",
              mostActiveDay: "Thursday",
            };

            set({ analytics: transformedAnalytics });
            return { success: true };
          } else {
            set({
              analyticsError: result.error || "Failed to fetch analytics",
            });
            return { success: false, error: result.error };
          }
        } catch (error) {
      ...
      fetchMeetingHistory: async (page = 1, limit = 20, filters = {}) => {
        ...
        try {
          ...
          const response = await fetch(
            `${import.meta.env.VITE_API_BASE_URL}/api/users/${user.id}/meetings?${queryParams}`,
            {
              headers: get().getAuthHeaders(),
            },
          );
          ...
          if (response.ok && result.success) {
            ...
            set({
              historyError: result.error || "Failed to fetch meeting history",
            });
            return { success: false, error: result.error };
          }
        } catch (error) {
      ...
      bookmarkMoment: async (meetingId, timestamp, note = "") => {
        try {
          const response = await fetch(
            `${import.meta.env.VITE_API_BASE_URL}/users/bookmarks`,
            {
              method: "POST",
              headers: get().getAuthHeaders(),
              body: JSON.stringify({
      ...
      removeBookmark: async (bookmarkId) => {
        try {
          const response = await fetch(
            `${import.meta.env.VITE_API_BASE_URL}/users/bookmarks/${bookmarkId}`,
            {
              method: "DELETE",
              headers: get().getAuthHeaders(),
            },
          ...
          if (response.ok && result.success) {
            set((state) => ({
              bookmarkedMoments: state.bookmarkedMoments.filter(
                (b) => b.id !== bookmarkId,
              ),
            }));
            return { success: true };
      ...
      addContact: async (userId) => {
        try {
          const response = await fetch(
            `${import.meta.env.VITE_API_BASE_URL}/users/contacts`,
            {
              method: "POST",
              headers: get().getAuthHeaders(),
      ...
      removeContact: async (contactId) => {
        try {
          const response = await fetch(
            `${import.meta.env.VITE_API_BASE_URL}/users/contacts/${contactId}`,
            {
              method: "DELETE",
              headers: get().getAuthHeaders(),
            },
      ...
      markFeatureIntroduced: (feature) => {
        set((state) => ({
          onboardingStatus: {
            ...
            featuresIntroduced: [
              ...state.onboardingStatus.featuresIntroduced,
              feature,
            ],
          },
        }));
      ...
      getParticipationTrend: () => {
        ...
        if (
          !Array.isArray(participationTrends) ||
          participationTrends.length < 2
        )
          return "stable";
        ...
        if (
          !recent[0] ||
          !recent[1] ||
          typeof recent[0].value !== "number" ||
          typeof recent[1].value !== "number"
        ) {
          return "stable";
        }
      ...
      getTodaysMeetingsCount: () => {
        ...
        return (
          dashboardData?.todaysMeetings?.length ||
          dashboardData?.quickStats?.todaysCount ||
          0
        );
      },
      ...
      getUpcomingMeetingsCount: () => {
        ...
        return (
          ...
          dashboardData?.quickStats?.upcomingCount ||
          0
        );
      },
      ...
      getUnreadNotificationsCount: () => {
        return (
          get().dashboardData.notifications?.filter((n) => !n.read).length || 0
        );
      },
