    ...
    (set, get) => ({
      ...
      addRecentMeeting: (meeting) => {
        set((state) => ({
          recentMeetings: [
            meeting,
            ...state.recentMeetings.filter(m => m.id !== meeting.id).slice(0, 9)
          ]
        }));
      },
      ...
      addRecentContact: (contact) => {
        set((state) => ({
          recentContacts: [
            contact,
            ...state.recentContacts.filter(c => c.id !== contact.id).slice(0, 9)
          ]
        }));
      },
      ...
      initializeFromSystem: () => {
        ...
        set((state) => ({
          theme: state.theme === 'system' ? (prefersDark ? 'dark' : 'light') : state.theme,
          accessibility: {
            ...state.accessibility,
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        if (state) {
          ...
          state.loading = Object.keys(state.loading || {}).reduce((acc, key) => {
            acc[key] = false;
            return acc;
          }, {});
