...
const useMeetingStore = create((set, get) => ({
  ...
  updateMeeting: async (meetingId, updateData) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/meetings/${meetingId}`,
        {
          method: "PUT",
          headers: get().getAuthHeaders(),
  ...
  startMeeting: async (meetingId) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/meetings/${meetingId}/start`,
        {
          method: "POST",
          headers: get().getAuthHeaders(),
        },
  ...
  endMeeting: async (meetingId) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/meetings/${meetingId}/end`,
        {
          method: "POST",
          headers: get().getAuthHeaders(),
        },
  ...
  getUpcomingMeetings: () => {
    ...
    return meetings.filter(
      (meeting) =>
        new Date(meeting.scheduled_start) > now &&
  ...
  admitFromWaitingRoom: (participantId) => {
    const participant = get().waitingParticipants.find(
      (p) => p.id === participantId,
    );
    if (participant) {
  ...
  removeAdminNotification: (notificationId) => {
    set((state) => ({
      adminNotifications: state.adminNotifications.filter(
