...
const useAdminStore = create((set, get) => ({
  ...
  fetchDashboardOverview: async () => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        set({
          dashboardError: result.error || "Failed to fetch dashboard overview",
        });
  ...
  fetchMeetingStats: async () => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/admin/meetings/stats`,
        {
          headers: get().getAuthHeaders(),
        },
      );
  ...
  fetchRecentMeetings: async () => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/admin/meetings?recent=true`,
        {
          headers: get().getAuthHeaders(),
        },
      );
      ...
      if (response.ok && result.success) {
        get().setDashboardData({
          recentMeetings: result.meetings || result.data,
        });
        return { success: true };
      } else {
        return { success: false, error: result.error };
  ...
  fetchUpcomingMeetings: async () => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/admin/meetings?upcoming=true`,
        {
          headers: get().getAuthHeaders(),
        },
      );
      ...
      if (response.ok && result.success) {
        get().setDashboardData({
          upcomingMeetings: result.meetings || result.data,
        });
        return { success: true };
      } else {
        return { success: false, error: result.error };
  ...
  inviteUser: async (invitationData) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/admin/users/invite`,
        {
          method: "POST",
          headers: get().getAuthHeaders(),
  ...
  createMeeting: async (meetingData) => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        return { success: true, meeting: result.meeting || result.data };
      } else {
  ...
  fetchAllOrganizations: async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/superadmin/organizations`,
        {
          headers: get().getAuthHeaders(),
        },
      );
      ...
      if (response.ok && result.success) {
        ...
        set({
          organizationError: result.error || "Failed to fetch organizations",
        });
        return { success: false, error: result.error };
      }
    } catch (error) {
  ...
  fetchSystemHealth: async () => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/superadmin/system/health`,
        {
          headers: get().getAuthHeaders(),
        },
      );
