...
class NotificationService {
  ...
  async sendSubscriptionToServer(subscription) {
    try {
      const response = await fetch(`${this.apiBaseUrl}/users/push-subscription`, {
        method: 'POST',
  ...
  handleSystemAlert(payload, notification) {
    const { title, message, alert_type, priority, action_url, action_label } = payload;
  ...
  async acceptMeetingInvitation(notificationId, joinUrl) {
    try {
      const response = await fetch(`${this.apiBaseUrl}/users/meeting-invitations/accept`, {
        method: 'POST',
        headers: this.getAuthHeaders(),
  ...
  async declineMeetingInvitation(notificationId) {
    try {
      const response = await fetch(`${this.apiBaseUrl}/users/meeting-invitations/decline`, {
        method: 'POST',
        headers: this.getAuthHeaders(),
        body: JSON.stringify({ notification_id: notificationId }),
      });
  ...
  async grantPermission(notificationId, permission) {
    try {
      const response = await fetch(`${this.apiBaseUrl}/admin/permissions/grant`, {
        method: 'POST',
        headers: this.getAuthHeaders(),
        body: JSON.stringify({ 
  ...
  async denyPermission(notificationId, permission) {
    try {
      const response = await fetch(`${this.apiBaseUrl}/admin/permissions/deny`, {
        method: 'POST',
        headers: this.getAuthHeaders(),
        body: JSON.stringify({ 
          notification_id: notificationId,
  ...
  async allowToSpeak(notificationId) {
    try {
      const response = await fetch(`${this.apiBaseUrl}/admin/hand-raise/allow`, {
        method: 'POST',
        headers: this.getAuthHeaders(),
        body: JSON.stringify({ notification_id: notificationId }),
      });
    ...
    if (this.notificationCallbacks.has(type)) {
      ...
      callbacks.forEach(callback => {
        try {
          ...
          console.error(`Error in notification callback for type ${type}:`, error);
        }
      });
    }
  }
  ...
  persistNotification(notification) {
    try {
      const stored = JSON.parse(localStorage.getItem('persistedNotifications') || '[]');
      stored.push({
        ...notification,
  ...
  loadPersistedNotifications() {
    try {
      const stored = JSON.parse(localStorage.getItem('persistedNotifications') || '[]');
      const recent = stored.filter(n => Date.now() - n.persistedAt < 24 * 60 * 60 * 1000); // 24 hours
  ...
  cleanupDisplayedNotifications() {
    ...
    if (this.displayedNotifications.size > this.maxDisplayedNotifications) {
      ...
      for (let i = 0; i < entries.length - this.maxDisplayedNotifications; i++) {
