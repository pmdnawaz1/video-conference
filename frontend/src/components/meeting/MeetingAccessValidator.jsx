...
const MeetingAccessValidator = ({ 
  ...
  const validateMeetingAccess = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.data.access.success && result.data.access.data.accessGranted) {
          if (result.data.access.data.waitingRoomRequired) {
            setValidationStep('waiting');
          } else if (result.data.time.success && result.data.time.data.canJoin) {
  ...
  const handleJoinMeeting = async () => {
    ...
    try {
      const result = await meetingAccessService.joinMeeting(meetingId, localJoinOptions);
    ...
    return (
      <div className="space-y-3">
        <div className="flex items-center justify-between p-3 border rounded-lg">
          ...
          <Badge variant={accessInfo.access.success ? 'default' : 'destructive'}>
            {accessInfo.access.success ? (accessInfo.access.data.accessGranted ? 'Granted' : 'Restricted') : 'Failed'}
          </Badge>
        </div>
        ...
        <div className="flex items-center justify-between p-3 border rounded-lg">
          ...
          <Badge variant={accessInfo.time.success && accessInfo.time.data.canJoin ? 'default' : 'secondary'}>
            {accessInfo.time.success ? (accessInfo.time.data.canJoin ? 'Available' : 'Waiting') : 'Unknown'}
          </Badge>
        </div>
        ...
        <div className="flex items-center justify-between p-3 border rounded-lg">
          ...
          <Badge variant={accessInfo.organization.success && accessInfo.organization.data.isMember ? 'default' : 'secondary'}>
            {accessInfo.organization.success ? (accessInfo.organization.data.isMember ? 'Member' : 'Guest') : 'Unknown'}
          </Badge>
        </div>
        ...
        <div className="flex items-center justify-between p-3 border rounded-lg">
          ...
          <Badge variant="outline">
            {accessInfo.history.success ? `${accessInfo.history.data.participationCount || 0} meetings` : 'Unknown'}
          </Badge>
        </div>
      </div>
    );
  ...
  const renderMeetingInfo = () => {
    if (!accessInfo?.access?.success || !accessInfo.access.data.meetingInfo) return null;
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div className="flex items-center gap-2">
              ...
              <span>{new Date(meeting.scheduled_start).toLocaleTimeString()}</span>
            </div>
            <div className="flex items-center gap-2">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              {localJoinOptions.audioEnabled ? <Mic className="w-4 h-4" /> : <MicOff className="w-4 h-4" />}
              <span className="text-sm">Microphone</span>
            </div>
            <Button
              variant={localJoinOptions.audioEnabled ? "default" : "outline"}
              size="sm"
              onClick={() => setLocalJoinOptions(prev => ({ ...prev, audioEnabled: !prev.audioEnabled }))}
            >
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              {localJoinOptions.videoEnabled ? <Camera className="w-4 h-4" /> : <CameraOff className="w-4 h-4" />}
              <span className="text-sm">Camera</span>
            </div>
            <Button
              variant={localJoinOptions.videoEnabled ? "default" : "outline"}
              size="sm"
              onClick={() => setLocalJoinOptions(prev => ({ ...prev, videoEnabled: !prev.videoEnabled }))}
            >
          ...
          <div className="flex items-center justify-between">
            ...
            <Button
              variant={localJoinOptions.screenShareEnabled ? "default" : "outline"}
              size="sm"
              onClick={() => setLocalJoinOptions(prev => ({ ...prev, screenShareEnabled: !prev.screenShareEnabled }))}
            >
  ...
  const renderTimeCountdown = () => {
    if (!accessInfo?.time?.success || !accessInfo.time.data.timeUntilStart || accessInfo.time.data.timeUntilStart <= 0) {
      return null;
    }

    const progress = Math.max(0, 100 - ((timeRemaining / accessInfo.time.data.timeUntilStart) * 100));
        ...
        <CardContent className="space-y-4">
          <div className="text-center">
            ...
            <div className="text-sm text-gray-500">
              {accessInfo.time.data.bufferTime && accessInfo.time.data.bufferTime > 0 && 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              {showAdvanced ? renderValidationStatus() : (
                <div className="text-center space-y-4">
                  {validationStep === 'ready' && (
                    <>
                      <CheckCircle className="w-12 h-12 text-green-500 mx-auto" />
                      <div>
                        <h3 className="font-medium text-green-700">Access Granted</h3>
                        <p className="text-sm text-gray-600">You can join this meeting now</p>
                      </div>
                    </>
                  )}
                  ...
                  {validationStep === 'waiting' && (
                    <>
                      ...
                      <div>
                        <h3 className="font-medium text-blue-700">Waiting Room Required</h3>
                        <p className="text-sm text-gray-600">You'll need to wait for admission</p>
                      </div>
                    </>
                  )}
                  ...
                  {validationStep === 'waiting_time' && (
                    <>
                      ...
                      <div>
                        <h3 className="font-medium text-yellow-700">Meeting Not Started</h3>
                        <p className="text-sm text-gray-600">Please wait for the scheduled time</p>
                      </div>
                    </>
                  )}
                  ...
                  {validationStep === 'denied' && (
                    <>
                      ...
                      <div>
                        <h3 className="font-medium text-red-700">Access Denied</h3>
                        <p className="text-sm text-gray-600">
                          {accessInfo?.access?.data?.reason || 'You do not have permission to join this meeting'}
                        </p>
                      </div>
                    </>
                  )}
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="space-y-3">
                ...
                {(validationStep === 'waiting_time' || validationStep === 'denied') && (
                  <Button 
                    variant="outline" 
                    className="w-full" 
                    size="lg"
