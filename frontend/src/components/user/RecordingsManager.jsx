...
const RecordingsManager = () => {
  ...
  const handleBookmarkAdd = async () => {
    ...
    const result = await bookmarkMoment(selectedRecording.meeting_id, timestamp, bookmarkNote);
  ...
  const filteredRecordings = savedRecordings?.filter(recording => {
    const matchesSearch = !searchTerm || 
      ...
      recording.organizer?.name.toLowerCase().includes(searchTerm.toLowerCase());
    
    const matchesFilter = filterType === 'all' || 
      (filterType === 'starred' && recording.is_starred) ||
      (filterType === 'recent' && new Date(recording.date) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000));
            ...
            <CardContent>
              <ScrollArea className="h-96">
                {isLoading ? (
                  ...
                  <div className="space-y-2">
                    {filteredRecordings.slice((currentPage - 1) * 10, currentPage * 10).map((recording) => (
                      <Card 
                        key={recording.id} 
                        className={`cursor-pointer transition-colors ${
                          selectedRecording?.id === recording.id ? 'border-blue-500 bg-blue-50 dark:bg-blue-950' : 'hover:bg-gray-50 dark:hover:bg-gray-800'
                        }`}
                        ...
                        <CardContent className="p-3">
                          <div className="space-y-2">
                            <div className="flex items-start justify-between">
                              <h4 className="font-medium text-sm line-clamp-2">{recording.title}</h4>
                              <div className="flex items-center gap-1">
                  ...
                  <div className="text-center py-8 text-muted-foreground">
                    ...
                    <p className="text-sm">Your meeting recordings will appear here</p>
                  </div>
                )}
              </ScrollArea>
            </CardContent>
                ...
                <CardContent className="p-0">
                  ...
                  <div className="p-6">
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        <h2 className="text-xl font-semibold mb-2">{selectedRecording.title}</h2>
                      ...
                      <div className="flex items-center gap-2">
                        <Button
                          ...
                          onClick={() => {
                            ...
                            selectedRecording.is_starred = !selectedRecording.is_starred;
                        ...
                        <Button variant="outline" size="sm" onClick={() => handleShare(selectedRecording)}>
                          <Share2 className="w-4 h-4 mr-1" />
                          Share
                        </Button>
                        <Button variant="outline" size="sm" onClick={() => handleDownload(selectedRecording)}>
                    ...
                    {selectedRecording.description && (
                      <p className="text-sm text-muted-foreground mb-4">{selectedRecording.description}</p>
                    )}
                    ...
                    <Tabs defaultValue="info" className="w-full">
                      ...
                      <TabsContent value="info" className="space-y-4">
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          <div>
                            ...
                            <div className="text-sm text-muted-foreground">{selectedRecording.organizer?.name}</div>
                          </div>
                          <div>
                            <div className="text-sm font-medium">File Size</div>
                            <div className="text-sm text-muted-foreground">{formatFileSize(selectedRecording.file_size)}</div>
                          </div>
                          <div>
                            <div className="text-sm font-medium">Quality</div>
                            <div className="text-sm text-muted-foreground">{selectedRecording.quality || 'HD'}</div>
                          </div>
                          <div>
                            <div className="text-sm font-medium">Format</div>
                            <div className="text-sm text-muted-foreground">{selectedRecording.format || 'MP4'}</div>
                          </div>
                        </div>
                      </TabsContent>
                            ...
                            <div className="space-y-2 text-sm">
                              {selectedRecording.transcript?.map((entry, index) => (
                                <div key={index} className="flex gap-3">
                                  ...
                                  <span className="font-medium min-w-24">{entry.speaker}:</span>
                      ...
                      {selectedRecording.chat_messages && (
                        <TabsContent value="chat">
                          <ScrollArea className="h-64 border rounded p-4">
                            <div className="space-y-3">
                              {selectedRecording.chat_messages.map((message, index) => (
                                <div key={index} className="flex gap-3">
                        ...
                        <div className="space-y-2">
                          {bookmarks.length > 0 ? (
                            bookmarks.map((bookmark, index) => (
                              <div key={index} className="flex items-center justify-between p-3 border rounded">
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    <div className="font-medium text-sm">{formatDuration(bookmark.timestamp)}</div>
                                    {bookmark.note && (
                                      <div className="text-xs text-muted-foreground">{bookmark.note}</div>
                                    )}
                                  </div>
                                </div>
                                <Button
                            ...
                            <div className="text-center py-8 text-muted-foreground">
                              ...
                              <p className="text-sm">Click the bookmark button to save moments</p>
                            </div>
                          )}
                        </div>
                      </TabsContent>
                      ...
                      {selectedRecording.shared_files?.length > 0 && (
                        <TabsContent value="files">
                          <div className="space-y-2">
                            {selectedRecording.shared_files.map((file, index) => (
                              <div key={index} className="flex items-center justify-between p-3 border rounded">
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    ...
                                    <div className="text-xs text-muted-foreground">
                                      {formatFileSize(file.size)} • Shared by {file.shared_by}
                                    </div>
                                  </div>
                                </div>
            ...
            <Card>
              <CardContent className="py-12">
                <div className="text-center">
                  ...
                  <h3 className="text-lg font-medium mb-2">Select a Recording</h3>
                  <p className="text-muted-foreground">
