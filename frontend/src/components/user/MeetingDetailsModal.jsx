...
import { 
  FiClock, FiUsers, FiMessageSquare, FiVideo, FiPlay, FiDownload, 
  FiShare2, FiCalendar, FiMic, FiMicOff, FiVideoOff, FiPhone, 
  FiThumbsUp, FiHeart, FiEye, FiCopy, FiExternalLink 
} from 'react-icons/fi';
...
const MeetingDetailsModal = ({ meeting, onClose }) => {
  ...
  const loadChatHistory = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/meetings/${meeting.id}/chat-history`);
  ...
  const loadParticipants = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/meetings/${meeting.id}/participants`);
      const data = await response.json();
      if (data.success) {
  ...
  const loadAnalytics = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/meetings/${meeting.id}/analytics`);
      const data = await response.json();
      if (data.success) {
      ...
      <div className="space-y-4">
        <div className="flex items-start justify-between">
          <div className="space-y-2">
            <h2 className="text-2xl font-bold text-foreground">{meeting.title}</h2>
            <div className="flex items-center space-x-4 text-sm text-muted-foreground">
              <div className="flex items-center space-x-1">
                <FiCalendar className="w-4 h-4" />
                <span>{format(new Date(meeting.scheduled_start), 'EEEE, MMMM dd, yyyy')}</span>
              </div>
              <div className="flex items-center space-x-1">
                <FiClock className="w-4 h-4" />
                <span>{format(new Date(meeting.scheduled_start), 'HH:mm')} - {format(new Date(meeting.scheduled_end), 'HH:mm')}</span>
              </div>
            </div>
          </div>
            ...
            <Card className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">{formatDuration(meetingDuration)}</p>
                </div>
            ...
            <Card className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">{meeting.participants?.length || 0}</p>
                </div>
            ...
            <Card className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">{meeting.totalMessages || 0}</p>
                </div>
            ...
            <Card className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <Badge className="mt-1" variant={meeting.status === 'completed' ? 'default' : 'secondary'}>
                    {meeting.status}
                  </Badge>
                </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center space-x-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {format(new Date(meeting.actual_start || meeting.scheduled_start), 'HH:mm:ss')}
                  </p>
                </div>
              </div>
              ...
              {meeting.keyEvents?.map((event, index) => (
                <div key={index} className="flex items-center space-x-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {format(new Date(event.timestamp), 'HH:mm:ss')} - {event.description}
                    </p>
                  </div>
                </div>
              ))}
              ...
              <div className="flex items-center space-x-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {format(new Date(meeting.actual_end || meeting.scheduled_end), 'HH:mm:ss')}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="text-center">
                <p className="text-2xl font-bold text-blue-600">
                  {Math.round(meeting.userParticipation?.attendancePercentage || 0)}%
                </p>
                ...
                <Card key={participant.id} className="p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <h4 className="font-medium">{participant.first_name} {participant.last_name}</h4>
                    ...
                    <div className="flex items-center space-x-4 text-sm">
                      <div className="text-center">
                        <p className="font-medium">{formatDuration(participant.duration || 0)}</p>
                        <p className="text-muted-foreground">Duration</p>
                      </div>
                      <div className="text-center">
                        <p className="font-medium">{participant.messages || 0}</p>
                        <p className="text-muted-foreground">Messages</p>
                      </div>
                      <div className="flex space-x-1">
                        {participant.hadVideo && <FiVideo className="w-4 h-4 text-green-500" />}
                        {participant.hadAudio && <FiMic className="w-4 h-4 text-blue-500" />}
                        {participant.wasHost && <Badge variant="outline">Host</Badge>}
                      </div>
                    </div>
                  </div>
                </Card>
                ...
                <div className="space-y-3">
                  {chatHistory.length === 0 ? (
                    <div className="text-center py-8">
                      ...
                      <p className="text-muted-foreground">No messages in this meeting</p>
                    </div>
                  ) : (
                          ...
                          {message.reactions && message.reactions.length > 0 && (
                            <div className="flex space-x-1 mt-2">
                              {message.reactions.map((reaction, rIndex) => (
                                <Badge key={rIndex} variant="outline" className="text-xs px-2 py-1">
        ...
        <TabsContent value="analytics" className="space-y-6">
          {isLoadingAnalytics ? (
            ...
            <div className="space-y-6">
              ...
              <Card className="p-6">
                <h3 className="text-lg font-semibold mb-4">Participation Over Time</h3>
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={analyticsData?.participationTimeline || []}>
                  ...
                  <div className="h-48">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        ...
                        >
                          {(analyticsData?.audioUsage || []).map((entry, index) => (
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie
                          ...
                          label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`}
                        >
                          {(analyticsData?.videoUsage || []).map((entry, index) => (
