...
const ReactionDisplay = ({ 
  ...
  const renderSidebarReactions = () => (
    <div className={`w-64 h-full bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm border-l overflow-hidden ${className}`}>
      <div className="p-4 border-b">
        <h3 className="font-semibold text-gray-900 dark:text-white">Live Reactions</h3>
        <p className="text-sm text-gray-500">Real-time participant reactions</p>
      </div>
      
      <div className="p-4 space-y-2 max-h-96 overflow-y-auto">
        <AnimatePresence>
          {activeReactions.slice(-10).reverse().map((reaction) => (
            <motion.div
              key={reaction.animationKey}
  ...
  const renderBottomReactions = () => (
    <div className={`fixed bottom-0 left-0 right-0 bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm border-t p-4 ${className}`}>
        ...
        <div className="flex gap-2 overflow-x-auto pb-2">
          <AnimatePresence>
            {activeReactions.slice(-20).map((reaction) => (
              <motion.div
                key={reaction.animationKey}
                className="flex-shrink-0 flex items-center gap-2 bg-gray-50 dark:bg-gray-800 rounded-lg px-3 py-2"
