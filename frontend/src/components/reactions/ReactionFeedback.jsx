...
const ReactionFeedback = ({
        ...
        if (autoHide) {
          setTimeout(() => {
            setVisibleFeedback((prev) =>
              prev.filter((f) => f.id !== reaction.id),
            );
          }, autoHideDelay);
        }
      }
  ...
  const renderFeedbackItem = (feedback) => (
    <motion.div
      ...
      className={`bg-white dark:bg-muted0 rounded-lg shadow-lg border p-3 min-w-[200px] ${
        feedback.type === "sent"
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            className="h-6 w-6 p-0 hover:bg-muted0 dark:hover:bg-muted0"
          >
            <X className="w-3 h-3" />
          </Button>
        </div>
      ...
      <Card className="mt-4">
        <CardContent className="p-4">
          <div className="flex items-center justify-between mb-3">
            <h4 className="text-sm font-semibold text-muted-foreground dark:text-white">
              Reaction Stats
            </h4>
            <Button
              variant="ghost"
          ...
          <AnimatePresence>
            {showStatsSummary && (
              ...
              >
                ...
                {popularReaction && (
                  <div className="flex items-center justify-between p-2 bg-muted dark:bg-muted0 rounded">
                    <div className="flex items-center gap-2">
                      <div className="text-lg">{popularReaction.emoji}</div>
                      <div>
                        <div className="text-xs font-medium">Most Popular</div>
                        <div className="text-xs text-muted-foreground">
                          {popularReaction.count} times
                        </div>
                      </div>
                    </div>
                ...
                <div className="flex items-center justify-between p-2 bg-muted dark:bg-muted0 rounded">
                  ...
                  <Badge
                    variant={engagementScore > 70 ? "default" : "secondary"}
                  >
                    {engagementScore}%
                  </Badge>
                </div>
