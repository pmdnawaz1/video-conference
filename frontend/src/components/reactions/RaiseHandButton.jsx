          ...
          {raisedHandsQueue.slice(0, 10).map((user, index) => (
            ...
            >
              <div className="flex items-center gap-2">
                <Badge
                  variant="outline"
                  className="w-6 h-4 text-xs p-0 flex items-center justify-center"
                >
                  {index + 1}
                </Badge>
          ...
          >
            ...
            >
              ...
              {raisedHandsCount > 0 && (
                <Badge
                  variant="secondary"
                  className="ml-1 h-5 min-w-[20px] text-xs p-0 flex items-center justify-center"
                >
                  {raisedHandsCount}
                </Badge>
              )}
            ...
            {isHandRaised && userPosition && (
              <Badge
                variant="outline"
                className="absolute -top-2 -right-2 bg-white dark:bg-muted0 text-xs h-5 min-w-[20px] p-0 flex items-center justify-center"
              >
                #{userPosition}
              </Badge>
            )}
        ...
        <TooltipTrigger asChild>
          ...
          >
            ...
            {isHandRaised && userPosition && (
              <Badge
                variant="outline"
                className="absolute -top-1 -right-1 bg-white dark:bg-muted0 text-xs h-4 min-w-[16px] p-0 flex items-center justify-center"
              >
                {userPosition}
              </Badge>
            )}
        ...
        <TooltipContent side="top" className="max-w-xs">
          <div className="space-y-2">
            {isHandRaised ? (
              <div>
                ...
                {autoLowerTimeout && (
                  <div className="text-xs text-orange-500">
                    Will auto-lower in{" "}
                    {Math.ceil(
                      (autoLowerTimeout - (Date.now() - timeRaised)) / 1000,
                    )}
                    s
                  </div>
                )}
              ...
              <div className="font-medium">
                Click to raise your hand and request to speak
              </div>
            )}
            {renderQueueTooltip()}
          </div>
